...
export function OutlineRoot({ plugin }: { plugin: NovelWriterPlugin }) {
	...
	useEffect(() => {
		const openChapter = (event: Event) => {
			...
			window.setTimeout(() => document.querySelector(`[data-nw-chapter-id="${id}"]`)?.scrollIntoView({ behavior: 'smooth', block: 'center' }), 0);
		};
		window.addEventListener('novel-writer:open-outline-chapter', openChapter);
		return () => window.removeEventListener('novel-writer:open-outline-chapter', openChapter);
	}, []);
	...
	return <div className="nw-outline-view nw-outline-single-column">
		...
		{actos.map(a => { const caps = capitulos.filter(c => c.id_acto === a.id_acto); return <section className="nw-outline-act" key={a.id_acto}>
			...
			{caps.map(c => <div className="nw-outline-chapter" key={c.id_capitulo} data-nw-chapter-id={c.id_capitulo}>
