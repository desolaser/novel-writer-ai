	...
	return (
		<div className="nw-modal-2col">
			<div className="nw-modal-left">
				...
				<button className="nw-btn nw-btn-primary" style={{ width: '100%', marginTop: 8 }} onClick={doCreate}>+ New detail</button>
				...
				{sel ? (
					<div className="nw-detalles-edit" style={{ display: 'flex', flexDirection: 'column', gap: 10, height: '100%', overflow: 'hidden' }}>
						<div style={{ display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0 }}>
							<h3 style={{ flex: 1, margin: 0 }}>Edit detail</h3>
						</div>
						<label>Name</label>
						<input className="nw-input" value={nameDraft} placeholder="Unnamed detail" onChange={e => handleNameChange(e.target.value)} onBlur={handleNameBlur} />
						<div style={{ display: 'flex', flexDirection: 'row', justifyContent: 'space-between' }}>
							<div className="nw-tab-bar nw-tab-bar-compact" style={{ flexShrink: 0 }}>
								...
								{sel.tipo_detalle === TipoDetalle.Dropdown && <button className={tab === 'opciones' ? 'active' : ''} onClick={() => setTab('opciones')}>Options</button>}
							</div>
							<button className="nw-btn nw-btn-danger" onClick={async () => { if (confirm('Delete detail?')) { await useNovelWriter.getState().deleteDetalle(sel.id_detalle); setSelected(null); } }} title="Delete"><Icon.Trash /></button>
						</div>
						<div style={{ flex: 1, overflowY: 'auto' }}>
							{tab === 'general' && (
								<div className="nw-entry-tab" style={{ gap: 8 }}><label>Detail type</label>
									...
									<label>Associated types (categories)</label>
					...
					<p className="nw-muted">Select or create a detail.</p>
				)}
			</div>
		</div>
	);
...
function DetalleOpciones({ detalle, store }: { detalle: any; store: any }) {
	...
	const addOpcion = async () => {
		const op: OpcionDetalle = {
			id_opcion_detalle: genId(), nombre: 'New option', color: PALETTE[0], orden: opciones.length, id_detalle: detalle.id_detalle,
	...
	return (
		<div className="nw-entry-tab" style={{ gap: 6 }}>
			...
			<p className="nw-muted" style={{ fontSize: 11 }}>Define the options for the dropdown/menu.</p>
			<div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
				{opciones.map((o: any, i: number) => (
					<div key={o.id_opcion_detalle} style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
						<button className="nw-btn nw-btn-icon" onClick={() => move(i, -1)} disabled={i === 0} title="Move up">▲</button>
						<button className="nw-btn nw-btn-icon" onClick={() => move(i, 1)} disabled={i === opciones.length - 1} title="Move down">▼</button>
						<input className="nw-input" style={{ flex: 1 }} value={o.nombre} onChange={async e => { const n = { ...o, nombre: e.target.value }; setOpciones(opciones.map(x => x.id_opcion_detalle === o.id_opcion_detalle ? n : x)); await useNovelWriter.getState().upsertOpcion(n); }} placeholder="Option name" />
						...
						<button className="nw-btn nw-btn-icon nw-btn-danger" onClick={() => { useNovelWriter.getState().deleteOpcion(o.id_opcion_detalle); setOpciones(opciones.filter(x => x.id_opcion_detalle !== o.id_opcion_detalle)); }} title="Delete"><Icon.Trash width={12} height={12} /></button>
					</div>
				))}
			</div>
			<div style={{ display: 'flex', gap: 8, marginTop: 8 }}>
				<button className="nw-btn nw-btn-primary" onClick={addOpcion}>+ Add Option</button>
				<button className="nw-btn" onClick={() => { const s = [...opciones].sort((a, b) => a.nombre.localeCompare(b.nombre)); s.forEach((o, k) => { o.orden = k; useNovelWriter.getState().upsertOpcion(o); }); setOpciones(s); }}>Sort alphabetically</button>
			</div>
		</div>
	);
}