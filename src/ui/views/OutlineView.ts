...
export class OutlineView extends ItemView {
	...
	getDisplayText() { return 'Novel Writer Outline'; }
	getIcon() { return 'list'; }
	async onOpen() { await initializeStore(this.plugin); const el = this.containerEl.children[1] as HTMLElement; el.empty(); el.addClass('novel-writer-root'); this.root = createRoot(el); this.root.render(React.createElement(OutlineRoot, { plugin: this.plugin })); }
